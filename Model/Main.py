...
from Model.utility.helper import *
from Model.utility.batch_test import *
from time import time

from Model.BPRMF import BPRMF
from Model.CKE import CKE
from Model.CFKG import CFKG
from Model.NFM import NFM
from Model.AKUPP import AKUPP
...
if __name__ == '__main__':
    ...
    *********************************************************
    ...
    if args.model_type in ['akupp', 'cfkg']:
    ...
    *********************************************************
    ...
    elif args.model_type in ['akupp']:
        model = AKUPP(data_config=config, pretrain_data=pretrain_data, args=args)
    ...
    *********************************************************
    ...
    if args.save_flag == 1:
        ...
        elif args.model_type in ['ncf', 'nfm', 'akupp']:
    ...
    if args.pretrain == 1:
        ...
        if ckpt and ckpt.model_checkpoint_path:
            ...
            if args.report != 1:
                ...
                # *********************************************************
                # save the pretrained model parameters of akupp (i.e., user & item & kg embeddings) for pretraining other models.
                if args.save_flag == -2:
                    ...
                    print('save the weights of akupp in path: ', temp_save_path)
                    exit()
    ...
    *********************************************************
    ...
    for epoch in range(args.epoch):
        ...
        """
        *********************************************************
        Alternative Training for akupp:
        ...
        if args.model_type in ['akupp']:
            ...
            if args.use_kge is True:
                ...
                for idx in range(n_A_batch):
                    ...
                    #get the rippleset of batchdata
                    ripple_set=data_generator.generate_ripple_set(A_batch_data)
                    #start the training with user preference and knowledge propagation
                    feed_dict = data_generator.generate_train_A_feed_dict(model, A_batch_data,ripple_set)

                    _, batch_loss, batch_kge_loss = model.train_A(sess, feed_dict=feed_dict)
        ...
        *********************************************************
        ...
        if args.verbose > 0:
            perf_str = 'Epoch %d [%.1fs + %.1fs]: train==[%.5f=%.5f ], recall=[%.5f, %.5f], ' \
                       ...
                       (epoch, t2 - t1, t3 - t2, loss, kge_loss, ret['recall'][0], ret['recall'][-1],
